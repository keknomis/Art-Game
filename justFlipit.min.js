...
(function(a, b) {
    ...
    let d, e, f, g = "justFlipIt",
        ...
        j = {};
    j.pluginName = "justFlipIt";
    a.extend(c.prototype, {
        init: function() {
            !0 === this.settings.FlipX ? (e = "_flip_", d = "back", j.frontClass = "_flip_", j.backClass = "back", this._flipIt(d, e)) : (e = "_flipY_", d = "backY", j.frontClass = "_flipY_", j.backClass = "backY", this._flipIt(d, e))
        },
        _flipIt: function(b, c) {
            let d = this.element[0].className;
            j.orginalTagName = this.element[0].tagName.toLowerCase(), j.oldClasses = d, j.orginalID = a(this.element[0]).attr("id");
            let e = this.element.first().removeClass(),
                f = a("<" + j.orginalTagName + " class=\"hover _justFlipIt_panel\">"),
                g = e.wrap(f);
            g.wrap(a("<div class=\"front\">")), a(this.element[0].parentElement).addClass(d);
            let h = this.elementWidth,
                i = this.elementHeight,
                k = this.elementBackgroundColor,
                l = this.element.first().clone().removeClass(),
                m = a("<div class=\"" + b + " " + d + "\">").append(l),
                n = this.element.parent().parent(),
                o = this.element.closest(".hover");
            n.css({
                width: h,
                height: i
            }).append(m), a.each(this.settings.Style, function(a, b) {
                b.el && ("self" === b.el.toLowerCase() ? n.css(b.style) : n.find(b.el).css(b.style))
            }), "" !== this.settings.Template && n.find("." + b).removeClass(d).html(this.settings.Template), !0 === this.settings.Click ? n.on("click", function(b) {
                b.preventDefault(), a(this).toggleClass(c)
            }) : "string" == typeof this.settings.Click ? n.find(this.settings.Click).on("click", function() {
                n.toggleClass(c)
    ...
    }), a.fn.justFlipIt = function(b) {
        return this.each(function() {
            a.data(this, "plugin_justFlipIt") || a.data(this, "plugin_justFlipIt", new c(this, b))
        })
    }, a.fn.destroy = function() {
        this.each(function(b, c) {
            let d, e = a(c).attr("id") === void 0 ? "" : a(c).attr("id"),
                f = a(c).attr("class");
            d = a(this).hasClass("front") ? a(this) : a(this).find(".front"), d.siblings().hasClass(j.backClass) ? (d.siblings().remove(), d.children().attr("class", j.oldClasses).unwrap().unwrap()) : (d.unwrap(), d.next().remove(), a(d.firstChild).attr("class", j.oldClasses).unwrap()), a.removeData(this, "plugin_justFlipIt")
        })
    }
